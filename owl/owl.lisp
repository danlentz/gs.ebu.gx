...
;; History
;; -------
;; 2009.11.03    file is renamed to OWL.rdf.
...
(defpackage :gx (:use :cl)
            (:export owl-same-p owl-different-p owl-equivalent-p disjoint-p owl-complement-p 
                     individual-p owl-equalp owl-class-p owl-thing-p OneOf intersection-of
...
(eval-when (:load-toplevel)
  (read-rdf-file #'addRdfXml #P"GX:ontology;owl.rdf"))
;;; ==================================================================================
...
(defun definitely-owl-equivalent-p (x y)
  ...
  (cond ((equal x y))         ; symbols, objects, and gx:uri. If both are equal, then equal.
        ...
        ((and (net.uri:uri-p x) (net.uri:uri-p y) (uri= x y))) ; or go through next
        ((and (uri-p x) (uri-p y))                             ; uri-string different but
         (definitely-owl-equivalent-p (uri-value x) (uri-value y))) ; <--
        ((and (uri-p x) (uri-boundp x)) (definitely-owl-equivalent-p (uri-value x) y))
        ((and (uri-p y) (uri-boundp y)) (definitely-owl-equivalent-p x (uri-value y)))
        ((and (consp x) (consp y))
...
(defun definitely-owl-same-p (x y &optional pairs)
  (cond ((and pairs
        ...
        ((inverse-functional-property-equal-p
          x y :test #'(lambda (a b) (definitely-%owl-same-p a b (cons (cons x y) pairs)))))
        ((and (symbolp x) (object? x) (symbolp y) (object? y))
         (definitely-%owl-same-p (symbol-value x) (symbol-value y) pairs))
        ...
        ((and (uri-p x) (uri-p y)) (uri= x y))    ; uris
        ((uri-p x) (definitely-%owl-same-p (uri-value x) y pairs))
        ((uri-p y) (definitely-%owl-same-p x (uri-value y) pairs))
        ((and (cl:typep x 'rdf:inLang) (cl:typep y 'rdf:inLang))
         (and (eq (lang x) (lang y)) (equal (content x) (content y))))
        ((and (cl:typep x 'rdf:XMLLiteral) (cl:typep y 'rdf:XMLLiteral))
         (and (eq (class-of x) (class-of y)) (equal (value-of x) (value-of y))))))
...
(defun definitely-owl-different-p (x y)
  ...
  (cond ((equal x y) nil)
        ...
        ((and (uri-p x) (uri-p y))
         (cond ((uri= x y) nil)
               (t (definitely-%owl-different-p (uri-value x) (uri-value y)))))
        ((uri-p y)
         (definitely-owl-different-p x (uri-value y)))
        ((uri-p x)
         (definitely-owl-different-p (uri-value x) y))
        (t nil)))
...
(defmethod update-instance-for-different-class :after
  ...
  (unless (cl:typep previous owl:Class)
    ...
    (let* ((old-supers (mop:class-direct-superclasses previous))
           (new-supers (make-this-supers current old-supers)))
      (unless (set-equalp old-supers new-supers)
