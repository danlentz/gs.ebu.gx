...
;; 2009.04.24    object? is defined, which is the same as resource?
...
;;   (require :namespace)
;;   (require :rdfboot)) ; end of eval-when
...
;;; (rdf-equalp (uri "http://somewhere") (uri "http://somewhere"))    -> true
;;; (rdf-equalp (uri "http://somewhere") (uri "http://some%20where")) -> false
...
;;; (setf (uri-value <http://somewhere>)
;;;   (addForm '(rdf:Description )))             -> #<|rdfs:Resource| :anonymous>
;;; (setf (uri-value <http://anotherplace>)
;;;   (addForm '(rdf:Description )))             -> #<|rdfs:Resource| :anonymous>
;;; (rdf-equalp <http://somewhere> <http://anotherplace>)             -> false
;;; (let ((*nonUNA* t))
...
(defun rdf-equalp (x y)
  ...
  (cond ((equal x y))                      ; cl:strings, symbols, objects, and gx:uri. If both are equal, then equal.
        ...
        ((and (net.uri:uri-p x) (net.uri:uri-p y) (uri= x y))) ; uri-string equal or go through next
        ...
        ((and (uri-p x) (uri-boundp x)) (rdf-equalp (uri-value x) y))
        ((and (uri-p y) (uri-boundp y)) (rdf-equalp x (uri-value y)))
        ...
        ((mop:class-finalized-p class)
         (cond ((member rdfs:Class (mop:class-precedence-list class) :test #'eq) t)
               ((and (string= (symbol-name (class-name class)) "Class")
                ...
                (error "Maybe OWL module is not loaded!"))
               (t nil)))
        ((labels ((walk-partial-cpl (c)
                    (let ((supers (mop:class-direct-superclasses c)))
...
(defun rdf-resolve (x &optional head-adding-stuff)
  ...
  (typecase x
    ...
    (uri (if (uri-value x) (uri-value x) x))
...
(defun rsc-object-p (x)
  ...
  (and (excl::standard-instance-p x)
       (not (eq x (load-time-value (find-class 'galaxy))))  ; not galaxy
...
(defun %instance-p (x)
  ...
  (and (excl::standard-instance-p x)
       ...
       (not (eq x (load-time-value (find-class 'galaxy))))))
...
;;;  <a shadow-class> names of multiple classes
...
(defun shadow-class-p (x)
  "returns true if <x> is an instance of shadow class.
   shadow-class is defined at RdfsObjects file."
  (eq (class-name (class-of x)) 'shadow-class))
...
(defun type-of (x)
  ...
  (case (cl:type-of x)
    ...
    (rdfs:Resource (cond ((shadow-class-p (class-of x)) (mapcar #'name (mclasses x)))
                         (t (cl:type-of x))))
    (_rdfsClass (cond ((eql x rdfs:Class) 'rdfs:Class)
                     (t (error "Another meta-metaclass than rdfs:Class:~S" x))))
    (otherwise (cond ((stringp x) 'xsd:string)
                     ((shadow-class-p (class-of x)) (mapcar #'name (mclasses x)))
                     (t (cl:type-of x))))))
...
;;; # If <c> is an instance of _rdfsClass, it implies <c> is rdfs:Class, then
...
(defun typep (object type)
  ...
  (typecase type
    (rdfs:Resource    ; type is an object in RDF universe including rdfs:Resource itself.
     (typecase object
       ...
       (net.uri:uri (cond ((string= (name type) "Ontology") (values t t))
                          ...
                          ((and (uri-p object) (uri-boundp object))
                           (%typep (uri-value object) type))
                          (t (values nil t))))
    ...
    (symbol (cond ((object? type) (typep object (symbol-value type)))
                  ...
                  (t (values nil t))))
    (uri (cond ((uri-boundp type) (typep object (uri-value type)))
               ((cl:typep object type) (values t t))
               (t (values nil t))))
    (cons (case (op type)
...
(defun disjoint-pairs-p (classes)
  (some #'(lambda (x)
            (some #'(lambda (y) (and (not (eq x y)) (disjoint-p x y)))
                  classes))
        classes))
