...
;;   (require :domainrange)
;;   (require :rdfskernel)
;;   (copy-readtable rdf::*standard-readtable* cl:*readtable*)
;; ) ; end of eval-when
      ...
      (net.uri:uri (assert (and (not (eq role t)) (not (eq role nil))))
                   (cond ((eq role 'rdf:about) (error "Cant happen!")) ; form2slot processes it before falling here
                         ...
                         (t (let ((name (uri2symbol form))     ; getting name of URI
                                  (uri (uri form)))            ; ensure interning and getting valued-uri 
                              (cond (name (setf (uri-value uri) (addForm name role)))
                                    ; un-named URI denotes anonymous object
                                    (t (if (uri-boundp uri) (uri-value uri)
                                         (setf (uri-value uri) (addForm `(rdfs:Resource ) role)))
                                       ))))))
      ;; typed literal???
              ...
              (cond ((lang? form) (setq lang-env form))
                    ...
                    (t ;; then form is a name
                     (let ((uri (symbol2uri form))
                       ...
                       (when uri (setf (uri-value uri) obj))
                       obj))))
      (rdf:inLang form)
                       ...
                       (let ((class
                              (cond ((or (null (car form)) (eq (car form) 'rdf:Description))
                                     (if (assoc 'rdf:type (cdr form))
                                         (symbol-value (cadr (assoc 'rdf:type (cdr form))))
                         ...
                         (assert obj)
                         (setf (resource-type-tag obj) (car form))
                         (let* ((name (name obj))
...
;;; * %addForm(_rdfsClass) - Error. Never happen because %addForm((eql rdfs:Class)) supersedes this.
...
;;; # <addObject>(_rdfsClass)  - accepts only rdfs:Class, the body is same as the main routine above.
;;; # <addObject>:around(rdfs:Class) - for OneOf processing, see OneOf module
;;; # <addObject>:around(_rdfsClass)  - same as <addObject>:around(rdfs:Class)
...
(defun addClass (classes name absts &optional islots)
  "create a class of metaclass <classes> with <name>, <absts>, and <islots>.
  ...
  (when (and name (not (symbol2uri name)) (documentation (symbol-package name) t))
    ...
    (flet ((ensure-multiple-classes (metas cls)
           ...
           (set-uri-value (obj)
              ...
              and push <obj> into *subjects-defined*"
                          (with-slots (rdf:about rdf:ID) obj
                            (cond ((slot-boundp obj 'rdf:about) (setf (uri-value (uri rdf:about)) obj))
                                  ((slot-boundp obj 'rdf:ID) (setf (uri-value (symbol2uri rdf:ID)) obj))))
      ...
      (cond ((null name)                                        ; anonymous
              ...
              (ensure-multiple-classes
               ...
               (if absts
                 ...
                 (apply #'mop:ensure-class-using-class nil nil
                        :metaclass (car classes)
                        initargs)))))
            ((not (object? name))                             ; new name
             (when (and (boundp name) (symbol-value name))      ; used name
               (error "DEBUG!")
            ...
            (t                                                  ; redefining
             ...
             (let ((obj (symbol-value name)))
               (cond ((every #'(lambda (cls) (cl:typep obj cls)) classes)
                      ;(format t "~%~S is an instance of ~S." obj classes)
                      (cond ((every #'(lambda (abst) (cl:subtypep obj abst)) absts)
                             (cond ((every #'(lambda (slot)
                                           ...
                                           islots)
                                    obj)
                                   (t ;(format t "~%Adding Class by redefinition1 ~S ~S ~S ~S" classes name absts islots)
                                    (apply #'reinitialize-instance obj initargs))))
                            (t ;(format t "~%Adding Class by redefinition2 ~S ~S ~S ~S" classes name absts islots)
                     ...
                     ((%instance-p obj)                          ; instance to class
                      ...
                      (ensure-multiple-classes
                       classes
                       (apply #'mop:ensure-class-using-class (find-class name) name
                              :direct-superclasses absts
                              :metaclass (car classes)
                              initargs)))
                     ((rdf-metaclass-p (car classes))
                      ;(format t "~%Adding Class in redefinition ~S ~S ~S ~S" classes name absts islots)
                      (cond ((and (null absts) (null initargs))
                             (cond ((subtypep (class-of obj) (car classes))
                                    (ensure-multiple-classes classes obj))
                                   ((subtypep (car classes) (class-of obj))
...
(defun addInstance (classes name &optional slots)
  "creates a new instance or redefines the instance with <name> and <slots>. 
  ...
  (let ((initargs
    ...
    (flet ((ensure-multiple-classes (classes obj)
            ...
            (set-uri-value (obj)
              ...
              (pushnew obj *subjects-defined*)
              obj))
      (cond ((null name)                   ; anonymous but may be ensured to multiple classes
              (set-uri-value
                (ensure-multiple-classes
                  classes (apply #'make-instance (car classes) initargs))))
        ((or (not (boundp name)) (null (symbol-value name)))
          ;; new name and may be ensured to multiple classes.
          ;;(format t "~%NEW NAME ~S" name)
          (set-uri-value
            (ensure-multiple-classes
              classes (apply #'make-instance (car classes) :name name initargs))))
        ((not (object? name))                              ; used name
          (warn "~&Bounded symbol ~S is dispatched for a resource." name)
          (set-uri-value
            (ensure-multiple-classes
              classes (apply #'make-instance (car classes) :name name initargs))))
        ((and (null slots) (eq (car classes) (class-of (symbol-value name))))
          (ensure-multiple-classes classes (symbol-value name)))
        (t
            ;; redefine (format t "~%Redefining ~S with classes:~S~%
            ;;slots:~S in addInstance" name classes slots) If name is
            ;;a blank node ID, definitely it is bound to an default
            ;;object.  Therefore the control always falls here, and
            ;;the object is anonymous.
          (let* ((obj (symbol-value name))
                  (mclasses (substitute rdfs:Resource |rdfs:Resource| (mclasses obj))))
            (cond ((every #'(lambda (cls) (cl:typep obj cls)) classes)
              ...
              ((null (cdr classes)) ; different single class, maybe forward ref to regular def
                (let ((newcls (car classes)))
                  (cond ((eq newcls |rdfs:Resource|)
                          (when initargs (apply #'reinitialize-instance obj initargs))
                          obj)
                    ((every #'(lambda (old) (cl:subtypep old newcls)) mclasses)) ; nothing done
              ...
              ((null (set-difference classes mclasses
                ...
                (when initargs (apply #'reinitialize-instance obj initargs))
                (ensure-multiple-classes classes obj))
              (t (let ((old-classes (get-mclasses-from (class-of obj))))
                   (error "Partly subtype but partly not subtype ~S ~S." classes mclasses)
                   (cond ((not (and (subsetp old-classes mclasses :test #'owl-equalp)
                                 (subsetp old-classes mclasses :test #'owl-equalp)))
                                        ;(format t "~%classes:~S" classes)
                                        ;(format t "~%Old-mclasses:~S" mclasses)
                                        ;(format t "~%Old-classes:~S" old-classes)
                           (error "Bingo1"))
                     (t (let ((new-classes (most-specific-concepts (append classes mclasses))))
...
(defmethod make-instance :around ((class rdfs:Class) &rest initargs)
  ...
  ;; make-instance (class . initargs) and make-instance (meta
                                       ;;. initags) (format t
                                       ;;"~%MAKE-INSTANCE:AROUND2 ~S
                                       ;;~S" class initargs)
  (cond ((null initargs) (call-next-method))
    (t (let ((new-domains (ensure-class-slotds class initargs))) ; slots are defined this class
         (cond ((null new-domains)
                 (call-next-method))
           ((null (cdr new-domains))
             (let ((new-domain (car new-domains)))
               (cond ((and (strict-class-p new-domain) (strict-class-p class)))
                 ((and (rdf-metaclass-p new-domain) (rdf-metaclass-p class)))
                 ((rdf-metaclass-p class)
                   (unless (cl:subtypep class new-domain)
                 ...
                 (t (error "Cant happen!")))
               (cond ((subsumed-p class new-domain)
                       (call-next-method))
                 ((subsumed-p (car new-domains) class)
                   (apply #'call-next-method new-domain initargs))
                 (t (warn "Multiple classing1 ~S with ~S"
                      (getf initargs :name) (cons class new-domains))
                   (apply #'call-next-method (make-shadow class (cons class new-domains)) initargs)
                   ))))
           ((notany #'(lambda (ty) (subsumed-p class ty)) new-domains)
             (error "Bingo2!")
             (let ((new-types (most-specific-concepts (nreverse (cons class new-domains)))))
               (warn "Multiple classing1 ~S with ~S" (getf initargs :name) new-types)
               (apply #'call-next-method (make-shadow class new-types) initargs)))
           (t (apply #'call-next-method class initargs)))))))
...
(defmethod reinitialize-instance :before ((instance rdfs:Resource) &rest initargs)
  ...
  ;;(format t "~%REINITIALIZE-INSTANCE:BEFORE(rdfs:Resource) ~S~% with
  ;;~S)" instance initargs)
  (let ((class (class-of instance)))
    (let ((new-domains (ensure-class-slotds class initargs))) ; new slots are defined this class
      (setq initargs
            (loop for (role filler) on initargs by #'cddr
                unless (eq filler :unbound)
                append (list role filler)))
      (cond ((null new-domains))                          ; nothing done
            ((null (cdr new-domains))
             (cond ((subsumed-p class (car new-domains))) ; nothing done
                   ((subsumed-p (car new-domains) class)
                    (change-class instance (car new-domains)))
                   (t (error "Bingo3!")
                     (warn "Multiple classing2 ~S with ~S"
                       (getf initargs :name) (cons class new-domains))
                      (change-class instance (make-shadow class (cons class new-domains))))))
            ((notany #'(lambda (ty) (subsumed-p class ty)) new-domains)
             (error "Bingo4!")
             (let ((new-types (most-specific-concepts (nreverse (cons class new-domains)))))
...
(defun ensure-class-slotds (class initargs)
  "checks properties in <initargs> and adds the new slot definitions to <class>.
   This function returns a list of domains for <class>, in which new domain may be
   included by new slots."
  (let ((properties (collect-props-from-initargs initargs)))
    (when properties
      ;(format t "~%ENSURE-CLASS-SLOTDS ~S~%  with ~S" class initargs)
      ...
      (let ((slot-names (collect-prop-names-from class))
        ...
        (when (setq new-props (set-difference properties slot-names :test #'eq))
          (loop for prop in new-props with new-domain
              when (property? prop)
              do ;(format t "~%    NEW-SLOT:~S adding to ~S." prop class)
                (reinitialize-instance
                 class
                 :direct-slots
                 `((:name ,prop :initargs (,prop) :type ,(or (get-range (symbol-value prop)) t)
                    ...
                    :subject-type ,class)))
                (setq operated t)
                (when (setq new-domain (get-domain (symbol-value prop)))
                  (cond ((consp new-domain)
                         (setq new-domains
                               (union (reverse (remove rdf:List new-domain)) new-domains)))
                        ((not (eql new-domain rdf:List))
                         (pushnew new-domain new-domains)))))
...
(defun read-in-lang-env (str)
  "reads <str> with language option in RDF, and returns an instance of rdf:inLang.
See also, <lang-env>."
