...
;;   (require :rdfshare)1  ) ; end of eval-when
   ...
   (lambda (condition stream)
     (let ((fcont (simple-condition-format-control condition))
       ...
       (format stream "invalid RDF parse: ~A: ~S at line ~S"
         (apply #'format nil fcont args) (expose-buf) (line-number stream))))))
                        ...
                        (cond ((char= (car (last pat)) q) pat)
                          (t (setq pat (coerce pat 'cl:string))
                            (let ((found (cdr (assoc pat *entity-decls* :test #'string=))))
                              (cond (found)
                                (t (error "There is no entity definition for ~A at line ~S."
                                     pat (line-number stream))
                                  )))))))))
                              ...
                              (let ((found (cdr (assoc pat *entity-decls* :test #'string=))))
                                (cond (found)
                                  (t (error "There is no entity definition for ~A at line ~S."
                                       pat (line-number stream)))))))
                      ((char= c #\<)
                        (error "Illegal AttValue with '<' at line number ~S"
                          (line-number stream)))
                      (t c)))))
    (when value
      (setq value
...
(defun read-Attribute-in-Description (stream)
  ...
  (let (Name)
    (cond ((match-pattern-p "xmlns:" stream) ; PrefixedAttName
           (error "'xmlns:' in description at line ~S" (line-number stream)))
...
(defun read-STag-or-EmptyElemTag-in-Description (stream)
  ...
  (cond ((read-pattern-p "<" stream)
        ...
        (t (error "Illegal STag at line number ~S." (line-number stream)))))
