...
(cl:defpackage :gx
  ...
  (:export uri uri-p uri-value set-uri-namedspace set-uri-namedspace-from-pkg
    get-uri-namedspace uri-namedspace uri2package uri2env uri2symbol irregular-name&pkg
    export-as-QName *base-uri* *default-namespace*  symbol2uri name-ontology nodeID?
    nodeID2symbol *uri2symbol-name-mapping-fun* *uri2symbol-package-mapping-fun*))
...
;;; See, <gx::double-angle-bracket-reader>.
;;; ----------------------------------------------------------------------------------
...
(defclass uri (net.uri:uri) ((value :accessor uri-value))
  (:documentation "tailored uri in SWCLOS that is a subclass of
  net.uri:uri and able to bind a value to, just like lisp symbol."))
...
;;;; URI APIs fixes
;;;
;;; Note that method <gx:uri> always interns <thing>, but <gx:parse-uri> does not intern it.
;;;

(defun parse-uri (thing &rest args)
  "when uri host exists but no uri path on <thing>, this method adds
   '/' to uri path.  This is required for the namespace interning. See
   also,  in ACL document, 
...
;;;
;;; Three methods are defined for generic function <uri>: when <thing> is a uri
;;; (instance of class net.uri:uri),  its interned value is returned. When <thing>
;;; is a string, <intern-uri> is applied to create a interned uri 
;;; from the string. 
;;;
;;; A uri is always interned for a given uri string, because the uniqueness is
;;; required for uri to be bound to a  value. This notion is same as lisp symbol.
...
;;;
;;; Every uri for RDF is globally unique in the world. Therefore, a
;;; uri in system must be unique.  The uniqueness of uri is assured in
;;; SWCLOS by interning a uri. Furthermore, every uri in RDF may be
;;; related to the corresponding QName, which consists of Prefix and
;;; LocalPart, if it exists.  Such a named uri must have a name space
;;; that coincide with Prefix. The prefix name space can be shared
;;; among uris that has the same Prefix so that LocalPart has a unique
;;; entry in each named space on Prefix. This machinery is very
;;; similar to interning mechanism of lisp symbol, which consists of a
;;; symbol name and package, and a symbol name is unique in the
;;; package.
;;; ----------------------------------------------------------------------------------
;;; (get-uri-namedspace "http://www.w3.org/2000/01/rdf-schema") ->
;;; #<uri-namedspace http://www.w3.org/2000/01/rdf-schema>
;;; (uri2package "http://www.w3.org/2000/01/rdf-schema") -> #<The rdfs
;;; package>
;;; ----------------------------------------------------------------------------------
;;;
;;; In most of cases, the mapping from a uri to a QName is
;;; algorithmically decidable. However, there may be cases that system
;;; cannot decide how a uri should be mapped to a QName. In such a
;;; case, a user must provide the mapping function for additional
;;; rules or giving each mapping by replying the query from system one
;;; by one.  Therefore, we need a maintenance device to keep irregular
;;; mapping for such cases.  Followings provide such a mapping device
;;; and the named uri namespace functionality.
;;;
;;; The first thing to be done is, in spite that whether the mapping
;;; is regular or irregular, to divide a uri into a Prefix part and a
;;; LocalPart part. The Prefix uri part is associated to a uri named
;;; space (which is also associated a symbol package of QName), in
;;; which LocalParts of uris which share the identical Prefix are
;;; associated to a symbol name of QName.
;;;
;;; Note that PrefixedAttName declaration in XML documents set the
;;; namespace with NCname (Prefix) and property value (associated
;;; prefix-uri). Note that DefaultAttName declaration in XML documents
;;; set the default namespace.  See Rdf module.

(defvar *NameSpaces* (make-uri-space)
  "hasharray where a Prefix relating uri part is interned. This space
   ensures the uniqueness of a Prefix relating uri by interning
   it. See <make-uri-space> in Allegro Common Lisp documentation.")
...
;;; In case of regular mapping from URI to QName, any device for
;;; mapping from local name to symbol name is needless.  However, in
;;; case of irregular mapping, the mapping from full URI to local name
;;; is needed in the namespace.  This book-keeping is done in the
;;; environment slot of <uri-namedspace>.

(defclass uri-namedspace (net.uri:uri)
  ((package :initform nil :accessor uri-namedspace-package)
    (env :initform nil :accessor uri-namedspace-env))
  ;; env is a uri to symbol-name assoc list for irregular symbol.
  (:documentation "A subclass of <net.uri:uri>. This instance has
additional two slots, i.e., an associated symbol package slot and uri
to symbol name mapping environment slot."))

(defun set-uri-namedspace (prefix-uri)
  "after interning <prefix-uri> to <*NameSpaces*>, change the class of
   <prefix-uri> from <net.uri:uri> to <uri-namedspace>. After that,
   symbol to uri mapping can be placed in this namespace."
...
(defun set-uri-namedspace-from-pkg (pkg)
  "supposing <pkg> has a documentation that is the same string as
   rendered <prefix-uri>, sets the <prefix-uri> as uri-namedspace, and
   puts this <pkg> into uri-namedspace-package slot."
...
(defun %uri2symbol (uri)
  "in case of <uri> with fragment, mapping is regular. Then, <uri>
   without fragment is Prefix part and fragment of <uri> is LocalPart
   part of <uri>. This function returns the QName of <uri> without
   consulting the LocalPart environment in .  its namespace. If there
   is no package information on <uri> without fragment part, a
   function bound to <*uri2symbol-package-mapping-fun*> is
   invoked. Note that uri-namedspace-env is unused in this regular
   case.  Note that QName symbol is automatically exported in this
   function."
  (declare (optimize (speed 3) (safety 1)))
...
(defun irregular-name&pkg (uri)
  "when the mapping is irregular, this function is called.  Firstly, a
   function bound to <*uri2symbol-name-mapping-fun*> is invoked with
   <uri> argument.  If it gives a symbol then the symbol is returned.
   If it gives a string, the string is used as symbol name in a
   package that is obtained from <uri> through <uri2package> or from a
   function bound to <*uri2symbol-package-mapping-fun*>.  When the
   package is newly obtained from <*uri2symbol-package-mapping-fun*>,
   the package is associated this <uri> itself.  Then, in the worst
   case, each irregular <uri> has its own namespace, as system cannot
   know general rules from one by one Q&A.  You had better provide a
   smarter application-oriented function on
   <*uri2symbol-name-mapping-fun*> that provides always an appropriate
   QName."
...
(defun default-uri2symbol-package-mapping-fun (uri)
  "This function is bound to <*uri2symbol-name-mapping-fun*> as
default. This function just makes a query for users."
...
(defun default-uri2symbol-name-mapping-fun (uri)
  "This function is bound to <*uri2symbol-package-mapping-fun*> as
   default. If <uri> has a uri path, then the returned value of
   <%%uri2symbol> is returned. Othewise a query is made for users."
...
(defun %%uri2symbol (uri)
  "Even if <uri> has no fragment, plausible separation is done by this
   function. In short, the file name or the most subfolder of <uri>
   path is taken as symbol name, and the remaining part of <uri> path
   is taken for namespace (package) association. If you can find some
   application specific rules for making QName, you had better program
   it as well as this function does."
  (let* ((path (parse-namestring (uri-path uri)))
    ...
    ;; ex: "http://somewhere/somedirectory/subdir/JohnSmith" -> #<uri
    ;; http://somewhere/somedirectory/subdir/> ex:
    ;; "http://somewhere/JohnSmith" -> #<uri http://somewhere/>
...
(defvar *default-namespace* nil
  "Default name space URI for XML parsing in current time. This value
  is set by <read-rdf-from-http> and <read-rdf-file>.")

(defvar *base-uri* nil
  "Base URI that is indicated in XML file. This value is set by
  <read-rdf-from-http>, <read-rdf-file>, and <read-rdf-from-string>.")
...
(defun uri-escape (str)
  ...
  (cond ((and (> (length str) 5) (string= "http:" (subseq str 0 5))) str)
    ;; this is for ontology URIs
...
(defun %uri-escape-for-percent (str)
  ...
  (let ((pos 0))
    (cond ((setq pos (position #\% str)) ;; found
            (concatenate 'cl:string (subseq str 0 pos) "%25"
              (%uri-escape-for-percent (subseq str (1+ pos)))))
          (t str))))
...
(defun uri-de-escape (str)
  ...
  (let ((pos 0))
    (cond ((setq pos (position #\% str :test #'char=))
            (let ((c (code-char (parse-integer
                                  (subseq str (1+ pos) (min (length str) (+ pos 3)))
                                  :radix 16))))
              (concatenate 'cl:string 
                (subseq str 0 pos)
                (string c)
  ...
  (let ((nc (peek-char nil stream t nil t)))
    (cond ((char= nc #\<)          ; double #\<
            ...
            (let* ((uri-str 
                     (coerce 
                       (loop with char
                         until (char= #\> (setq char (read-char stream)))
                         collect char)
                       'cl:string))
                    (uri (uri uri-str))
                    (symbol nil))
              ...
              (cond ((uri-boundp uri)
                ...
                ((setq symbol (uri2symbol uri))
                  (let ((obj nil))
                    ...
                    (list 'quote obj)))
                (t ;; blank node
                  (setf (uri-value uri) (make-instance '|rdfs:Resource|))
                  (list 'quote (uri-value uri))))))
...
(defun symbol2QNameString (symbol)
  ...
  (cond ((eql (symbol-package symbol) (uri-namedspace-package *default-namespace*))
         (symbol-name symbol))
    (t (concatenate 'cl:string (package-name (symbol-package symbol)) ":"
         (symbol-name symbol)))))
...
(defun symbol2uri (symbol)
  ...
  ;; (when (multiple-value-bind (sym in/ex) (intern (string symbol)
  ;; (symbol-package symbol)) (declare (ignore sym)) (not (eq in/ex
  ;; :external))) (error "Internal symbol ~S is designated in
  ;; symbol2uri." symbol))
  (or (and (boundp symbol)
    ...
    (let* ((name (uri-escape (symbol-name symbol)))
      ...
      (or (cdr (assoc symbol env))
        (and ns
          (cond
            ((and (pathname-name (merge-pathnames (net.uri:uri-path ns)))
               (null (net.uri:uri-fragment ns)))
              (uri 
                (copy-uri (net.uri:parse-uri (net.uri:render-uri ns nil)) :fragment name)))
            (t ;; no fragment but name
              (uri
                (copy-uri (net.uri:parse-uri
                            (concatenate 'cl:string (net.uri:render-uri ns nil) name)))))))
        ))))
...
(defun make-unique-nodeID (str)
  (declare (optimize (speed 3) (safety 1)))
  "makes a unique node ID from <str>. Namely, adds numbers at the end
of <str> and makes unique symbol."
