...
(def (special-variable e) *dtrace-print-length* 7)
(def (special-variable e) *dtrace-print-level*  4)
(def (special-variable e) *dtrace-print-circle* t)
(def (special-variable e) *dtrace-print-pretty* nil)
(def (special-variable e) *dtrace-print-array* *print-array*)

(def (special-variable e) *dtraced-functions* nil)
(defvar *trace-level* 0)

(def (macro e) dtrace (&rest function-names)
  "Turns on detailed tracing for specified functions.  Undo with DUNTRACE."
  (if (null function-names)
      (list 'quote *dtraced-functions*)
      (list 'quote (mapcan #'dtrace1 function-names))))

(def (macro) :dtrace (&rest function-names)
  "Turns on detailed tracing for specified functions.  Undo with DUNTRACE."
  (if (null function-names)
      (list 'quote *dtraced-functions*)
      (list 'quote (mapcan #'dtrace1 function-names))))
...
(defun dtrace1 (name)
  ...
  (when (iterate::special-form? name)
    (format *error-output*
  ...
  (if (macro-function name)
      (trace-macro name)
      (trace-function name))
  (setf *dtraced-functions* (nconc *dtraced-functions* (list name)))
  (list name))
...
(def (macro) with-dtrace-printer-settings (&body body)
...
(def (macro e) duntrace (&rest function-names)
  "Turns off tracing for specified functions.  
   With no args, turns off all tracing."
  (setf *trace-level* 0)  ;; safety precaution
  (list 'quote
	(mapcan #'duntrace1 (or function-names *dtraced-functions*))))


(def (macro) :duntrace (&rest function-names)
  "Turns off tracing for specified functions.  
   With no args, turns off all tracing."
  (setf *trace-level* 0)  ;; safety precaution
  (list 'quote
...
(defun duntrace1 (name)
  ...
  (setf *dtraced-functions* (delete name *dtraced-functions*))
...
#+swank
(def (function e) fetch-arglist (fn)
  (swank::arglist fn)) 
