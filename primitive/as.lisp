...
;;;;; seq.lisp
;;;;;
;;;;;   a general-purpose type-conversion utility
;;;;;   based on folio - the Bard runtime by mikel evins
;;;;;
...
(defmethod as ((class (eql 'cl:string)) (thing cl:symbol) &key &allow-other-keys)
  (if (symbol-package thing)
      (concatenate 'string (package-name (symbol-package thing)) "::" (symbol-name thing))
      (concatenate 'string "#:|" (symbol-name thing) "|")))
